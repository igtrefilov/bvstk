...
## Оглавление
  ...
  - [Введение](#введение)
    - [Назначение прошивки](#Назначение%20прошивки)
    - [Поддерживаемое железо и ограничения](#Поддерживаемое%20железо%20и%20ограничения)
    - [Термины и обозначения](#Термины%20и%20обозначения)
  - [Архитектура системы](#Архитектура%20системы)
    - [Общая схема модулей](#Общая%20схема%20модулей)
    - [Потоки/задачи FreeRTOS](#Потоки/задачи%20FreeRTOS)
    - [Порядок инициализации](#Порядок%20инициализации)
  - [Окружение разработки](#Окружение%20разработки)
    - [Требования](#требования)
    - [Входные HW‑артефакты](#Входные%20HW‑артефакты)
    - [Структура репозитория](#Структура%20репозитория)
  - [Сборка](#сборка)
    - [Быстрый старт](#Быстрый%20старт)
    - [Переменные сборки](#Переменные%20сборки)
    - [Артефакты и структура vitis_ws](#Артефакты%20и%20структура%20vitis_ws)
  - [Запуск и отладка](#Запуск%20и%20отладка)
    - [Запуск по JTAG](#Запуск%20по%20JTAG)
    - [JTAG step‑debug (VSCode/GDB)](#JTAG%20step‑debug%20%28VSCode/GDB%29)
    - [Подключение к TCP‑консоли](#Подключение%20к%20TCP‑консоли)
  - [Сеть (lwIP)](#Сеть%20%28lwIP%29)
    - [Инициализация интерфейса](#Инициализация%20интерфейса)
    - [Настройка IP/MAC и сохранение](#Настройка%20IP/MAC%20и%20сохранение)
    - [Смена IP и восстановление доступа](#Смена%20IP%20и%20восстановление%20доступа)
  - [Файловые системы (FatFs)](#Файловые%20системы%20%28FatFs%29)
    - [Тома и пути](#Тома%20и%20пути)
    - [Монтирование и автоформатирование](#Монтирование%20и%20автоформатирование)
    - [Разметка QSPI FS](#Разметка%20QSPI%20FS)
    - [Web UI в flash:/www/](#Web%20UI%20в%20flash:/www/)
  - [Конфигурация (JSON / config_store)](#Конфигурация%20%28JSON%20/%20config_store%29)
    - [Расположение и приоритеты](#Расположение%20и%20приоритеты)
    - [Дефолты и генерация](#Дефолты%20и%20генерация)
    - [Миграция legacy → primary](#Миграция%20legacy%20→%20primary)
    - [Сохранение и целостность](#Сохранение%20и%20целостность)
  - [PL‑ядра](#pl-ядра)
    - [Назначение и место в системе](#Назначение%20и%20место%20в%20системе)
    - [Общая схема PS ↔ PL core](#Общая%20схема%20PS%20↔%20PL%20core)
    - [Общий протокол обмена и ограничения](#Общий%20протокол%20обмена%20и%20ограничения)
    - [Очереди/ISR/worker tasks](#Очереди/ISR/worker%20tasks)
    - [Конфигурация и политики доступа](#Конфигурация%20и%20политики%20доступа)
    - [Диагностика и отладка](#Диагностика%20и%20отладка)
    - [PL‑ядро I2C (bvstk_i2c)](#PL‑ядро%20I2C%20%28bvstk_i2c%29)
      - [Модель устройств и JSON‑формат](#Модель%20устройств%20и%20JSON‑формат)
      - [Политики и persisted settings](#Политики%20и%20persisted%20settings)
      - [Autopoll и кэш](#Autopoll%20и%20кэш)
      - [Управление (консоль/HTTP)](#Управление%20%28консоль/HTTP%29)
    - [PL‑ядро SMI/MDIO (bvstk_smi)](#PL‑ядро%20SMI/MDIO%20%28bvstk_smi%29)
      - [Модель PHY и JSON‑формат](#Модель%20PHY%20и%20JSON‑формат)
      - [Политики и persisted settings](#Политики%20и%20persisted%20settings-1)
      - [Autopoll и обработка событий](#Autopoll%20и%20обработка%20событий)
      - [Управление (консоль/HTTP)](#Управление%20%28консоль/HTTP%29-1)
  - [TCP‑консоль (порт 8888)](#TCP‑консоль%20%28порт%208888%29)
    - [Обзор и правила ответов](#Обзор%20и%20правила%20ответов)
    - [Команды](#команды)
  - [HTTP‑сервер (порт 80)](#HTTP‑сервер%20%28порт%2080%29)
    - [Роутинг и форматы ответов](#Роутинг%20и%20форматы%20ответов)
    - [/api/*](#api)
    - [Файловый API](#Файловый%20API)
    - [Раздача Web UI](#Раздача%20Web%20UI)
  - [Веб‑ресурсы и деплой](#Веб‑ресурсы%20и%20деплой)
    - [Структура web/assets](#Структура%20web/assets)
    - [Загрузка в flash:/www/](#Загрузка%20в%20flash:/www/)
  - [Приложения](#приложения)
    - [Карта директорий](#Карта%20директорий)
    - [Таблица портов/протоколов](#Таблица%20портов/протоколов)
    - [Примеры команд и запросов](#Примеры%20команд%20и%20запросов)
...
- **bear** — для генерации `compile_commands.json` (IntelliSense/F12 по BSP):
  ...
  - затем: `./scripts/gen_compile_commands.sh`
- Инструкции:
...
- `scripts/` — вспомогательные скрипты (VSCode/инструменты):
  - `scripts/gen_compile_commands.sh` — генерация `compile_commands.json` через `bear` (для VSCode навигации по BSP)
  - `scripts/jtag_prepare_debug.tcl` — XSCT подготовка JTAG для отладки (PL + `ps7_init` + halt core0)
  - `scripts/arm-none-eabi-gdb.sh` — враппер для поиска `arm-none-eabi-gdb` (удобно для VSCode)
...
- TCP‑консоль: [TCP‑консоль → Команды](#команды)
- HTTP JSON‑API: [HTTP‑сервер → /api/*](#api)
- HTTP файловый доступ и tar‑деплой: [HTTP‑сервер → Файловый API](#Файловый%20API)
- Web UI и загрузка в `flash:/www/`: [Веб‑ресурсы и деплой](#Веб‑ресурсы%20и%20деплой)
