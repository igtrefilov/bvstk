...
и пересоберите библиотеку. Это compatibility-патч для неблокирующей обработки
и завершения однофайловых и рекурсивных SCP-сессий: без него OpenSSH `scp` может получить
...
Если в качестве назначения указать существующий каталог (`/sd`, `/sd:/`, `/flash` или `/flash:/`), файл или каталог будет сохранён в нём под своим исходным именем. Для каталога нужен ключ `-r`. Пути с `..`, неизвестные тома и вложенность более 16 уровней отклоняются.
