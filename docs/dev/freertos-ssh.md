...
TCP-консоль на `8888` продолжает работать параллельно. SSH в текущем варианте является инженерной консолью, а не полноценной Unix-оболочкой: SFTP/SCP, shell history и tab completion через SSH пока не реализованы. Передача файлов остаётся доступна через существующие файловые и HTTP-интерфейсы.
