...
[4. Основные архитектурные блоки](#4-основные-архитектурные-блоки)
	...
	[4.4. Сетевой уровень](#44-сетевой-уровень)
		[4.4.1. Архитектурная роль и границы ответственности](#441-архитектурная-роль-и-границы-ответственности)
		[4.4.2. Жизненный цикл и публикация готовности](#442-жизненный-цикл-и-публикация-готовности)
		[4.4.3. Источники сетевой конфигурации](#443-источники-сетевой-конфигурации)
		[4.4.4. Модель данных и сетевое состояние](#444-модель-данных-и-сетевое-состояние)
		[4.4.5. Взаимодействие с другими подсистемами](#445-взаимодействие-с-другими-подсистемами)
...
Детальный runtime ниже относится к полному FreeRTOS-варианту. Общий PL/service
контракт и текущая граница между FreeRTOS и Neutrino описаны отдельно в
`multi-os.md`.

Документ предназначен для использования как верхнеуровневый архитектурный
reference при анализе и изменении прошивки.
...
сетевой runtime на базе lwIP, внешние сервисы управления `TCP`, опциональный
`SSH`, `HTTP` и `DCP2`, а также программные подсистемы работы с PL-ядрами.
...
| Ethernet                     | Обеспечивает сетевую связность, необходимую для работы сервисов `TCP`, опционального `SSH`, `HTTP` и `DCP2`.                                                                                                |
...
`src/apps/freertos/main.c`. Именно в ней задаётся верхнеуровневая стартовая
последовательность
...
Практически это даёт три основных пути взаимодействия: командный shell через
TCP или опциональный SSH, HTTP и внутренний SD API. Оба shell-транспорта
используют общий слой `fs_shared`, вызывая `fs_shared_fs_ls()`,
...
`src/hardware/boards/ax7020/bvstk_qspi_layout.h` файловое окно FatFs задаётся
как диапазон, начинающийся с `QSPI_FS_BASE_BYTES` и занимающий
`QSPI_FS_SIZE_BYTES`. По умолчанию первые
...
Жизненный цикл подсистемы конфигурации в текущей реализации может быть
представлен следующим образом:

```mermaid
flowchart TB
    main["main()"]
    start["start_config_store()"]
    task["config_task()"]
    wait["Ожидание готовности qspi_fs"]
    prep["Подготовка путей и каталогов<br/>flash:/config/..."]
    load["Загрузка конфигурации<br/>fallback / миграция / парсинг"]
    publish["Публикация ready-state<br/>s_ready = 1"]
    users["Потребители конфигурации"]

    main --> start --> task --> wait --> prep --> load --> publish --> users
```

Подсистема конфигурации запускается из `main()` вызовом
`start_config_store()`. На этом этапе сама загрузка конфигурации ещё не
выполняется синхронно в контексте стартовой функции. Вместо этого
`start_config_store()` сбрасывает внутреннее состояние, очищает runtime-структуры
и поднимает отдельную задачу `config_task()`, внутри которой и происходит
дальнейшая работа подсистемы.

Первый этап внутри `config_task()` — ожидание готовности `qspi_fs`. Это
ожидание ограничено по времени и нужно для того, чтобы дать тому `flash:/`
возможность смонтироваться до того, как система окончательно решит, что
сохранённая конфигурация недоступна. После этого подсистема строит рабочие
пути primary и fallback-layout, а при доступном `flash:/` подготавливает
конфигурационное пространство: создаёт каталоги `flash:/config`,
`flash:/config/i2c` и `flash:/config/smi`, при необходимости записывает
встроенные дефолты и выполняет одноразовую миграцию из legacy-путей
`flash:/configs/...`.

Следующий этап — собственно загрузка конфигурации. На нём `config_store`
читает сетевой конфиг, наборы конфигов `PL` подсистем, разбирает JSON и
переводит его во внутренние структуры в памяти. Если часть данных отсутствует
или недоступна, подсистема заполняет соответствующие области встроенными
значениями по умолчанию. Тем самым к концу этого шага формируется не просто
набор файлов, а уже собранное рабочее конфигурационное состояние.

Публикация готовности происходит только после завершения этого цикла. Когда
`config_task()` заканчивает подготовку внутреннего состояния, он выставляет
`s_ready = 1`, и именно это состояние становится архитектурным сигналом для
остальных подсистем, что конфигурация уже загружена и опубликована. После
этого вызовы `config_store_get_*()`, `config_store_find_*()` и
`config_store_wait_ready()` становятся штатной точкой взаимодействия между
`config_store` и его потребителями.

#### 4.3.3. Источники конфигурации и приоритеты

Подсистема `config_store` собирает итоговое конфигурационное состояние не из
одного источника, а из нескольких уровней с разным приоритетом. В упрощённом
виде эта иерархия может быть представлена так:

```mermaid
flowchart TB
    primary["Primary layout<br/>flash:/config/..."]
    legacy["Legacy fallback<br/>flash:/configs/..."]
    defaults["Встроенные дефолты<br/>в прошивке"]

    runtime["Итоговое рабочее состояние<br/>config_store in RAM"]

    primary -->|приоритет 1| runtime
    legacy -->|приоритет 2| runtime
    defaults -->|приоритет 3| runtime
```

Порядок приоритета источников в текущей архитектуре таков: сначала
`config_store` опирается на primary-layout `flash:/config/...`, затем при его
отсутствии может использовать legacy-layout `flash:/configs/...`, и только
если оба варианта недоступны, переходит на встроенные дефолты в прошивке.

Это соответствие можно кратко представить в виде таблицы:

| Источник | Что это означает | Когда используется |
|---|---|---|
| `Primary layout` | Основной актуальный конфигурационный layout в `flash:/config/...` | Используется в первую очередь, если нужный конфиг существует в primary-каталоге |
| `Legacy fallback` | Совместимый fallback-layout в `flash:/configs/...` | Используется только если primary-версии нет, и нужен fallback или миграция со старой структуры |
| `Дефолты` | Встроенные конфигурационные значения из прошивки (`default_configs.h`) | Используются, если конфиги на `flash:/` недоступны или отсутствуют |

Основным persistent-источником конфигурации в текущей архитектуре считается
primary-layout `flash:/config/...`. Именно туда `config_store` записывает
актуальные версии `network.json` и конфигов `PL` подсистем, и именно этот
layout должен рассматриваться как нормальная форма хранения конфигурации
устройства.

Если нужный файл отсутствует в primary-layout, подсистема может использовать
legacy-layout `flash:/configs/...` как совместимый fallback. В коде это
реализовано через построение пары путей primary/fallback и чтение «первого
доступного» варианта. Архитектурно важно, что legacy-структура не является
равноправной альтернативой primary-layout: она служит для совместимости и
миграции, но не должна иметь приоритет над актуальным размещением
`flash:/config/...`.

Если ни primary, ни legacy-источник недоступны, `config_store` использует
встроенные значения по умолчанию, зашитые в прошивку через
`default_configs.h`. Это нижний уровень иерархии источников: он гарантирует,
что система сможет сформировать рабочее конфигурационное состояние даже при
пустом или неготовом `flash:/`.

Отдельно важно различать persistent-состояние и текущее рабочее состояние.
После завершения загрузки остальные части системы работают уже не с файлами
напрямую, а с опубликованными структурами `config_store` в памяти. Вызовы
`config_store_set_*()` изменяют именно это текущее состояние, тогда как
`config_store_save_*()` фиксируют его в primary-layout на `flash:/`. Из-за
этого между рабочей конфигурацией системы и сохранённой конфигурацией на
носителе может существовать временной разрыв.

#### 4.3.4. Модель данных и состав конфигурации

`config_store` хранит несколько отдельных
конфигурационных областей, каждая из которых представлена собственной
внутренней структурой данных. После
загрузки система работает уже не с JSON-файлами, а с нормализованными
структурами в оперативной памяти, которые и образуют рабочую модель
конфигурации устройства.

В текущей реализации состав этой модели можно представить так:

| Конфигурационная область | Внутренняя форма        | Что описывает                                                                           |
| ------------------------ | ----------------------- | --------------------------------------------------------------------------------------- |
| Сеть                     | `network_config_t`      | MAC-адрес, IPv4-адрес, маску, gateway и признаки наличия этих значений                  |
| `PL`-подсистемы          | `i2c_device_config_t[]`, `smi_phy_config_t[]` | описание устройств и PHY, политики доступа, autopoll и persisted settings |

Сетевой домен в этой модели является самым простым. Он хранится в единственном
экземпляре `network_config_t` и содержит рабочие сетевые параметры устройства.
Архитектурно это не просто снимок файла `network.json`, а уже подготовленная
структура, которую затем напрямую использует сетевой уровень при настройке
интерфейса.

Конфиги `PL`-подсистем устроены богаче и содержат не только статические
идентификаторы устройств, но и прикладные правила их использования.

| Тип данных | Примеры |
|---|---|
| Идентификация и адресация | `name`, `addr_7b`, `phy_addr` |
| Границы модели устройства | `reg_count`, `max_value_code` |
| Политики доступа | `policy`, `whitelist`, `blacklist`, `write_allow`, `write_deny` |
| Профили наблюдения | `autopoll_enabled`, `autopoll_regs`, delays |
| Persisted-настройки | `settings[]` |

Из этого следует важная архитектурная особенность: `config_store` хранит не
только “параметры подключения”, но и политику поведения подсистем. Для
`PL`-части это означает, что конфигурация описывает не просто то, какие
устройства существуют, но и то, как с ними разрешено работать, какие регистры
можно изменять, какие значения нужно применять при старте и какие регистры
следует наблюдать в фоне.

#### 4.3.5. Взаимодействие с другими подсистемами

Роль подсистемы конфигурации
раскрывается через связи с хранением, сетевым уровнем, `PL`-подсистемами и
сервисами управления. В упрощённом виде эти связи могут быть представлены так:

```mermaid
flowchart LR
    
    subgraph cfg["config_store"]
        direction TB
        net_cfg["Сетевой конфиг"]
        pl_cfg["Конфиги PL-подсистем"]
        policy["Политики и настройки"]
    end

    net["Сетевой уровень"]
    pl["PL-подсистемы"]

    subgraph mgmt["Сервисы управления"]
        direction TB
        shell["TCP shell"]
        ssh["SSH shell<br/>(опционально)"]
        http["HTTP API"]
    ...
    classDef ext fill:#e8f1ff,stroke:#3b82f6,stroke-width:1.5px,color:#222;
    class shell,ssh,http,dcp2 ext;
```


Для сетевого уровня `config_store` является источником базовых параметров
интерфейса: MAC-адреса, IPv4-адреса, маски и gateway. После публикации
готовности именно он даёт сетевой подсистеме исходное рабочее состояние,
которое затем используется при настройке интерфейса и при дальнейшем
управлении сетью.

Для `PL`-подсистем его роль ещё шире. Через `config_store` в runtime попадают
описания устройств, политики доступа, настройки `autopoll` и persisted
settings. Это означает, что он связывает файловое хранение конфигурации с
прикладной логикой `I2C`, `SMI` и других подсистем, работающих с внешними
устройствами через `PL`.

Отдельный класс взаимодействий образуют `TCP/SSH shell`, `HTTP API` и `DCP2`.
Shell-транспорты и HTTP не только читают опубликованную конфигурацию, но и могут изменять
её через `config_store_set_*()` и сохранять обратно в `flash:/` через
`config_store_save_*()`. `DCP2` в текущем коде взаимодействует прежде всего с
конфигами `PL`-подсистем и связанными с ними политиками. Поэтому
`config_store` служит общей
точкой согласования между persistent-данными, рабочим состоянием в памяти и
реально существующими внешними интерфейсами управления.

### 4.4. Сетевой уровень

Сетевой уровень отвечает за подъём IP-связности
устройства и создаёт инфраструктурную основу для сервисов управления. Именно
через него после загрузки конфигурации становятся доступны `TCP` shell,
опциональный `SSH`, `HTTP` и `DCP2`.

Этот уровень занимает промежуточное положение между
`config_store`, из которого он получает исходные сетевые параметры, и внешними
сетевыми сервисами, которые опираются на уже поднятый интерфейс.

#### 4.4.1. Архитектурная роль и границы ответственности

Сетевой уровень в архитектуре `bvstk` выполняет роль общего слоя сетевой
связности устройства. Он получает исходные параметры интерфейса из
`config_store`, поднимает рабочий Ethernet-интерфейс и тем самым создаёт
условия, при которых внешние сервисы управления становятся реально доступны по
сети.

Концептуально сетевой уровень здесь рассматривается в логике модели `OSI`,
где отдельно выделяются физический уровень, `MAC`, собственно сетевой уровень
и транспортный уровень. На этой основе его можно представить как вложенную
абстракцию нескольких слоёв:

```mermaid
flowchart TB
    subgraph phy["Eth PHY"]
        direction TB
        phy_hw["Внешний PHY платы<br/>Realtek RTL8211E"]

        subgraph mac["MAC-уровень"]
            direction TB
            mac_mod["Модули и обвязка<br/>xemacps / xadapter"]
            mac_fn["API и параметры PS GEM<br/>xemac_add()<br/>xemacif_input()<br/>XPAR_XEMACPS_0_BASEADDR"]
            cfg["config_store<br/>источник MAC/IP/gateway"]

            subgraph net["Сетевой уровень"]
                direction TB
                net_fn["Инициализация и обслуживание сетевого стека<br/>lwip_init()<br/>xemac_add()<br/>netif_set_default()<br/>netif_set_up()<br/>lan_thread()<br/>xemacif_irq_thread()<br/>xemacif_input()"]

                subgraph upper["Транспортный"]
                    direction TB
                    upper_fn["Запуск сетевых сервисов<br/>start_tcp_server()<br/>start_ssh_server()<br/>start_http_server()<br/>start_dcp2_server()"]
                end
            end
        end
    end
    ...
    classDef focus fill:#fff2cc,stroke:#b58900,stroke-width:2px,color:#222;
    class net,upper focus;
```

В границы ответственности этого уровня входит инициализация и поддержание
рабочего сетевого runtime, связанного с `netif`, MAC-адресом и IPv4-параметрами
устройства. Он отвечает за то, чтобы опубликованная конфигурация интерфейса
превратилась в реальную сетевую доступность устройства и чтобы прикладные
сервисы могли опираться на уже поднятую сеть.

На уровне слоёв полезно отдельно различать `PHY` и `MAC`. В текущем проекте
`PHY` представлен внешним Ethernet PHY аппаратной платформы. На стороне
аппаратной платформы используется `Ethernet PHY Realtek RTL8211E`, через
который `PS GEM` подключается к физической линии связи. На уровне `MAC` и GEM
прошивка уже работает со своей программной обвязкой: здесь используются
компоненты `xemacps` / `xadapter`, а также API и аппаратные параметры PS GEM,
такие как `xemac_add()`, `xemacif_input()` и `XPAR_XEMACPS_0_BASEADDR`.

При этом сетевой уровень не хранит конфигурацию сам по себе и не заменяет
`config_store` как источник параметров. Он также не реализует прикладную
логику `TCP shell`, опционального `SSH`, `HTTP` и `DCP2`: эти сервисы используют уже поднятый
сетевой интерфейс, но не входят в состав самого сетевого уровня. По этой
причине его следует рассматривать как отдельную инфраструктурную подсистему
между конфигурацией, Ethernet-интерфейсом и внешними сетевыми сервисами.

#### 4.4.2. Жизненный цикл и публикация готовности

#### 4.4.3. Источники сетевой конфигурации

#### 4.4.4. Модель данных и сетевое состояние

#### 4.4.5. Взаимодействие с другими подсистемами
