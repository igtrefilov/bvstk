...
Отдельный класс взаимодействий образуют `TCP shell`, `HTTP API` и `DCP2`.
Shell и HTTP не только читают опубликованную конфигурацию, но и могут изменять
...
через него после загрузки конфигурации становятся доступны `TCP`
shell, `HTTP` и `DCP2`.
        ...
        subgraph mac["MAC-уровень"]
            ...
            subgraph net["Сетевой уровень"]
                ...
                subgraph upper["Транспортный"]
                    direction TB
                    upper_fn["Запуск сетевых сервисов<br/>start_tcp_server()<br/>start_http_server()<br/>start_dcp2_server()"]
                end
            end
        end
    end
...
логику `TCP shell`, `HTTP` и `DCP2`: эти сервисы используют уже поднятый
