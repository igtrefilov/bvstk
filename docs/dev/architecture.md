...
С архитектурной точки зрения `SD` следует рассматривать как дополнительный
...
На `SD` целесообразно размещать данные, связанные с обменом с внешним миром,
ручной загрузкой и выгрузкой файлов, резервными копиями, архивами и другими
временными или переносимыми материалами. В отличие от `QSPI`, который ближе
к встроенному постоянному состоянию системы, `SD` играет роль съёмного и
менее жёстко связанного с устройством носителя.

В этом смысле архитектурная роль `SD` определяется не столько самим фактом
наличия ещё одного физического накопителя, сколько наличием отдельного
эксплуатационного файлового пространства, удобного для внешних операций и
сопровождения устройства.

##### 4.2.2.4. Готовность и режим деградации

Готовность подсистемы `SD` в текущей архитектуре имеет мягкий характер.
Отсутствие карты, ошибка инициализации или невозможность монтирования не
должны останавливать запуск остальных частей системы и не переводят устройство
в состояние полной неработоспособности.

При недоступности `SD` система продолжает функционировать за счёт остальных
подсистем, прежде всего встроенного хранения на `QSPI` и сервисов управления.
В этом случае деградация проявляется локально: файловые операции, завязанные
на `sd:/`, остаются недоступными до тех пор, пока носитель не будет успешно
инициализирован и примонтирован.

Такая модель делает `SD` опциональной по отношению к базовой
работоспособности устройства, но штатно поддерживаемой частью общей
