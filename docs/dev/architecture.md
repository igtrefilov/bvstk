...
##### 4.2.3.4. Готовность и режим деградации

Готовность подсистемы `QSPI` в текущей архитектуре важнее, чем готовность
`SD`, но и она не является абсолютным условием старта всей прошивки. Если том
`flash:/` не удалось поднять, система всё равно может продолжить запуск, однако
переходит в более жёсткий режим деградации.

Главное последствие связано с `config_store`. При старте он ограниченное время
ждёт готовности `qspi_fs`, а если том так и не становится доступен, переходит
на встроенные дефолтные конфигурации, зашитые в прошивку. Это позволяет
системе продолжить работу, но означает потерю доступа к сохранённому
постоянному состоянию на flash.

Второе последствие относится к HTTP и пользовательским ресурсам. При
неготовом `flash:/` становятся недоступны файловые HTTP-операции над
`/flash/...`, `GET /api/qspi` сообщает о неготовности подсистемы, а раздача
Web UI из `flash:/www/...` перестаёт быть штатно доступной. TCP-консоль и
другие сервисы управления при этом могут продолжить работу, но встроенное
файловое пространство и связанные с ним данные будут ограниченно доступны.

Таким образом, `QSPI` в архитектуре `bvstk` не является единственной точкой
полного отказа системы, но относится к подсистемам повышенной критичности.
Недоступность `flash:/` не обязательно останавливает boot path, однако заметно
сильнее влияет на конфигурацию, web-поверхность и постоянное состояние
устройства, чем недоступность `sd:/`.

#### 4.2.4. Общий слой файлового доступа

#### 4.2.5. Логические файловые устройства

#### 4.2.6. Готовность и режимы деградации
