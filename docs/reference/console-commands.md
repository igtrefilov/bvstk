...
| Команда | Назначение |
|---|---|
| `sd-pl debug` | вывести регистры состояния, точные CMD55/ACMD41 и 32 последних SPI-обмена |

В трассе поля имеют формат `[state tx rx pins]`. Команда не запускает новую
операцию с картой и не изменяет содержимое носителя.
