...
| `0x0100` | `ERR_FS_NOT_FOUND` | FS: том или объект отсутствует |
| `0x0101` | `ERR_FS_NOT_READY` | FS: том не готов |
| `0x0102` | `ERR_FS_BAD_HANDLE` | FS: дескриптор недействителен или истёк |
| `0x0103` | `ERR_FS_TYPE` | FS: тип объекта не соответствует режиму |
| `0x0104` | `ERR_FS_IO` | FS: ошибка ввода-вывода |
| `0x0105` | `ERR_FS_CHANGED` | FS: обнаружено изменение читаемого файла |

Файловый сервис использует отдельное отображение ошибок backend. Прежнее
отображение common API для остальных сервисов не меняется. Ошибочный FS-ответ
не содержит данных после status. `ERR_FS_NOT_READY` допускает ограниченные
повторы; BAD_HANDLE требует нового OPEN, CHANGED — нового чтения источника.
