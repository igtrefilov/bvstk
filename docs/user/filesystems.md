...
FreeRTOS-маршрутизатор файлов поддерживает SD и QSPI FatFs.

| Псевдоним | FatFs root | Роль |
|---|---|---|
| `sd:/` | `0:/` | съёмные и обменные файлы |
...
| `sd:/` | пользовательские и обменные файлы |
...
После старта SD и QSPI монтируются отдельными задачами. Сеть может отвечать раньше, чем тома станут готовы; перед файловой операцией проверяйте результат `ls` или API `/api/fs`.

При результате FatFs `FR_NO_FILESYSTEM` код пытается выполнить `f_mkfs`. Операция создаёт новую FAT-разметку на выбранном томе. Перед подключением носителя с данными подготовьте резервную копию.
...
Путь с `sd:/` или `flash:/` выбирает том явно. Относительные пути разрешаются относительно текущего каталога сессии.
...
Для классического SCP используется `-O`. `/sd` и `/sd:/` выбирают SD, `/flash` и `/flash:/` — QSPI.
...
| пустой каталог | проверить правильный том и возможное автоформатирование |
