...
#ifdef XPAR_XSDPS_NUM_INSTANCES
#define DISKIO_SD_PDRV_COUNT	XPAR_XSDPS_NUM_INSTANCES
#else
#define DISKIO_SD_PDRV_COUNT	1
#endif
...
static DRESULT qspi_write_sector(LBA_t sector, const BYTE *buff)
{
    ...
    memcpy(qspi_sector_cache + offset, buff, DISKIO_SECTOR_SIZE);
    if (qspi_flash_erase_sector(block_addr) != XST_SUCCESS) {
        return RES_ERROR;
    }
    if (qspi_flash_program(block_addr, qspi_sector_cache, QSPI_FLASH_SECTOR_SIZE) != XST_SUCCESS) {
        return RES_ERROR;
    }
    return RES_OK;
}
...
)
{
	...
	if (pdrv < DISKIO_SD_PDRV_COUNT) {		/* SD/eMMC interface */
#ifdef XPAR_XSDPS_NUM_INSTANCES
...
)
{
	...
	if (pdrv == DISKIO_QSPI_PDRV) {
		...
		return (qspi_flash_read(addr, buff, bytes) == XST_SUCCESS) ? RES_OK : RES_ERROR;
	}
#endif
...
)
{
	...
	if (pdrv == DISKIO_QSPI_PDRV) {
		for (UINT i = 0; i < count; ++i) {
			DRESULT res = qspi_write_sector(sector + i, buff + (i * DISKIO_SECTOR_SIZE));
			if (res != RES_OK) {
				return res;
			}
		}
