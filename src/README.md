...
## Оглавление
  ...
  - [Введение](#введение)
    ...
    - [Поддерживаемое железо и ограничения](#поддерживаемое-железо-и-ограничения)
    - [Термины и обозначения](#термины-и-обозначения)
  - [Архитектура системы](#архитектура-системы)
    - [Общая схема модулей](#общая-схема-модулей)
    - [Потоки/задачи FreeRTOS](#потоки-задачи-freertos)
    - [Порядок инициализации](#порядок-инициализации)
  - [Окружение разработки](#окружение-разработки)
    - [Требования](#требования)
    - [Входные HW‑артефакты](#входные-hw-артефакты)
    - [Структура репозитория](#структура-репозитория)
  - [Сборка](#сборка)
    - [Быстрый старт](#быстрый-старт)
    - [Переменные сборки](#переменные-сборки)
    - [Артефакты и структура vitis_ws](#артефакты-и-структура-vitis_ws)
  - [Запуск и отладка](#запуск-и-отладка)
    - [Запуск по JTAG](#запуск-по-jtag)
    - [Подключение к TCP‑консоли](#подключение-к-tcp-консоли)
  - [Сеть (lwIP)](#сеть-lwip)
    - [Инициализация интерфейса](#инициализация-интерфейса)
    - [Настройка IP/MAC и сохранение](#настройка-ip-mac-и-сохранение)
    - [Смена IP и восстановление доступа](#смена-ip-и-восстановление-доступа)
  - [Файловые системы (FatFs)](#файловые-системы-fatfs)
    - [Тома и пути](#тома-и-пути)
    - [Монтирование и автоформатирование](#монтирование-и-автоформатирование)
    - [Разметка QSPI FS](#разметка-qspi-fs)
    - [Web UI в flash:/www/](#web-ui-в-flash-www)
  - [Конфигурация (JSON / config_store)](#конфигурация-json-config_store)
    - [Расположение и приоритеты](#расположение-и-приоритеты)
    - [Дефолты и генерация](#дефолты-и-генерация)
    - [Миграция legacy → primary](#миграция-legacy-primary)
    - [Сохранение и целостность](#сохранение-и-целостность)
  - [PL‑ядра](#pl-ядра)
    - [Назначение и место в системе](#назначение-и-место-в-системе)
    - [Общая схема PS ↔ PL core](#общая-схема-ps-pl-core)
    - [Общий протокол обмена и ограничения](#общий-протокол-обмена-и-ограничения)
    - [Очереди/ISR/worker tasks](#очереди-isr-worker-tasks)
    - [Конфигурация и политики доступа](#конфигурация-и-политики-доступа)
    - [Диагностика и отладка](#диагностика-и-отладка)
    - [PL‑ядро I2C (bvstk_i2c)](#pl-ядро-i2c-bvstk_i2c)
      - [Модель устройств и JSON‑формат](#модель-устройств-и-json-формат)
      - [Политики и persisted settings](#политики-и-persisted-settings)
      - [Autopoll и кэш](#autopoll-и-кэш)
      - [Управление (консоль/HTTP)](#управление-консоль-http)
    - [PL‑ядро SMI/MDIO (bvstk_smi)](#pl-ядро-smi-mdio-bvstk_smi)
      - [Модель PHY и JSON‑формат](#модель-phy-и-json-формат)
      - [Политики и persisted settings](#политики-и-persisted-settings-1)
      - [Autopoll и обработка событий](#autopoll-и-обработка-событий)
      - [Управление (консоль/HTTP)](#управление-консоль-http-1)
  - [TCP‑консоль (порт 8888)](#tcp-консоль-порт-8888)
    - [Обзор и правила ответов](#обзор-и-правила-ответов)
    - [Команды](#команды)
  - [HTTP‑сервер (порт 80)](#http-сервер)
    - [Роутинг и форматы ответов](#роутинг-и-форматы-ответов)
    - [/api/*](#api)
    - [Файловый API](#файловый-api)
    - [Раздача Web UI](#раздача-web-ui)
  - [Веб‑ресурсы и деплой](#веб-ресурсы-и-деплой)
    - [Структура web/assets](#структура-web-assets)
    - [Загрузка в flash:/www/](#загрузка-в-flash-www)
  - [Приложения](#приложения)
    - [Карта директорий](#карта-директорий)
    - [Таблица портов/протоколов](#таблица-портов-протоколов)
    - [Примеры команд и запросов](#примеры-команд-и-запросов)
...
- **HTTP файловый API** — см. раздел [HTTP‑сервер → Файловый API](#файловый-api).
- **Web UI** (статика из `flash:/www/`) — см. раздел [Web UI в flash:/www/](#web-ui-в-flash-www).
...
Где что смотреть:
- правила роутинга статики и `Content-Type` — раздел [HTTP‑сервер → Раздача Web UI](#раздача-web-ui)
- исходники UI на хосте и способы загрузки в `flash:/www/` — раздел [Веб‑ресурсы и деплой](#веб-ресурсы-и-деплой)
...
- Для принудительного чтения/записи прямо в PHY используйте `PUT /api/diag/i2c/read|write` (см. [PL‑ядра → Диагностика и отладка](#диагностика-и-отладка)).
...
1) **`/sd/...`, `/flash/...`, `/tar/...`** — файловый API.
   - Подробности по маппингу/ограничениям путей и tar‑режиму — в разделе [Файловый API](#файловый-api).

2) **Web UI (статика из `flash:/www/`)** — только `GET`.
   - Подробности по маппингу статики и `Content-Type` — в разделе [Раздача Web UI](#раздача-web-ui).
...
- Отдача файлов и tar описана в разделах [Файловый API](#файловый-api) и [Раздача Web UI](#раздача-web-ui).
...
- HTTP файловый доступ и tar‑деплой: [HTTP‑сервер → Файловый API](#файловый-api)
- Web UI и загрузка в `flash:/www/`: [Веб‑ресурсы и деплой](#веб-ресурсы-и-деплой)
