...
- QSPI flash (Winbond W25Q series) is mirrored into the same FatFs stack at `1:/`; `cd flash` picks that disk in the shell and formats a fresh flash before exposing files (`src/qspi_fs/*`).
...
- `cd flash` / `cd sd` — swap the shell between the flash and SD filesystems, reusing the same helpers for whichever disk is selected.
