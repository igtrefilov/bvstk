...
- **HTTP файловый API** — см. раздел [HTTP‑сервер → Файловый API](#http-faylovyy-api).
- **Web UI** (статика из `flash:/www/`) — см. раздел [Web UI в flash:/www/](#web-ui-v-flashwww).
...
Где что смотреть:
- правила роутинга статики и `Content-Type` — раздел [HTTP‑сервер → Раздача Web UI](#http-razdacha-web-ui)
- исходники UI на хосте и способы загрузки в `flash:/www/` — раздел [Веб‑ресурсы и деплой](#web-resursy-i-deploy)
...
**HTTP: конфигурация**
- Подробности по `GET/PUT /api/i2c` (форматы JSON, примеры) — в разделе [HTTP‑сервер → /api/*](#http-api).
- Для принудительного чтения/записи прямо в PHY используйте `PUT /api/diag/i2c/read|write` (см. [PL‑ядра → Диагностика и отладка](#pl-diagnostika-i-otladka)).
...
0) **`/api/*`** — JSON‑API.
   ...
   - Подробности по эндпоинтам/форматам — в разделе [/api/*](#http-api).

1) **`/sd/...`, `/flash/...`, `/tar/...`** — файловый API.
   - Подробности по маппингу/ограничениям путей и tar‑режиму — в разделе [Файловый API](#http-faylovyy-api).

2) **Web UI (статика из `flash:/www/`)** — только `GET`.
   - Подробности по маппингу статики и `Content-Type` — в разделе [Раздача Web UI](#http-razdacha-web-ui).
...
- Отдача файлов и tar описана в разделах [Файловый API](#http-faylovyy-api) и [Раздача Web UI](#http-razdacha-web-ui).
...
- TCP‑консоль: [TCP‑консоль → Команды](#tcp-komandy)
- HTTP JSON‑API: [HTTP‑сервер → /api/*](#http-api)
- HTTP файловый доступ и tar‑деплой: [HTTP‑сервер → Файловый API](#http-faylovyy-api)
- Web UI и загрузка в `flash:/www/`: [Веб‑ресурсы и деплой](#web-resursy-i-deploy)
