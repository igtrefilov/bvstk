...
int main(void)
{
    ...
    status = bvstk_i2c_core_init(&i2c_core, NULL, NULL);
    if (status != BVSTK_OK) {
        fprintf(stderr, "bvstkd: I2C initialization failed: %s\n",
                bvstk_status_string(status));
        return 1;
    }
    status = bvstk_i2c_service_init(&i2c,
                                    &i2c_core,
                                    NULL,
                                    &default_device,
                                    1U,
    ...
    if (status != BVSTK_OK) {
        fprintf(stderr, "bvstkd: I2C service initialization failed: %s\n",
                bvstk_status_string(status));
        bvstk_i2c_core_shutdown(&i2c_core);
        return 1;
    }
    ...
    if (status != BVSTK_OK) {
        fprintf(stderr, "bvstkd: SMI initialization failed: %s\n",
                bvstk_status_string(status));
        bvstk_i2c_service_shutdown(&i2c);
        bvstk_i2c_core_shutdown(&i2c_core);
        return 1;
    }
