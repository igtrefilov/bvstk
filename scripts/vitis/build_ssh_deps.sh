...
# OpenSSH 9.x uses SFTP for plain `scp` and normally transfers up to 32 KiB
# per READ/WRITE request. Keep the SFTP payload size aligned with that client;
# the PL SD driver still splits each FatFs operation into its own 4 KiB
# controller batches.
WOLFSSH_CFLAGS="$SSH_DEPS_CFLAGS -DNO_FILESYSTEM -DWOLFSSH_FATFS -DWOLFSSH_BVSTK_FATFS -DWOLFSSH_LWIP -DWOLFSSL_LWIP -DWOLFSSH_TERM -DNO_TERMIOS -DWOLFSSH_SCP_USER_CALLBACKS -DWOLFSSH_MAX_SFTP_RW=32768 -DWOLFSSH_MAX_SFTP_RECV=4096 -I$SSH_DEPS_BSP_INCLUDE -I$SSH_DEPS_FATFS_INCLUDE -I$SSH_DEPS_XILINX_INCLUDE -I$SSH_DEPS_FREERTOS_INCLUDE -I$SSH_DEPS_LWIP_INCLUDE -I$SSH_DEPS_LWIP_CONTRIB_INCLUDE -I$WOLFSSL_INSTALL/include"
